...
public class Context_2_mybatis {
	...
	//생성자 주입
	public Context_2_mybatis(DataSource dw) {
		ds = dw;
	}
	
	//(SqlSessionFactory클래스)Factory객체가 DB에 접근하는 방법을 알고 있는 객체
	...
	//SQL을 처리하는 마이바티스 메서드를 호출할 때, SqlSessionTemplate은 Sqlsession이 현재의 스프링 트랜잭션에서
	//사용될 수 있도록 보장해준다.
	
	//SqlSessionTemplate은 필요한 시점에서 세션을 닫고, Commit하거나 Rollback하는 것을 포함한 Session의 LifeCycle을 관리한다.
